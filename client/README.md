...
- Expanded diagnostics: argument counts against builtin and script signatures, macro arity, unreachable code, unassigned and unused variables, duplicate imports, duplicate case labels, assignment used as a condition, inheritance cycles, `...` placement, and a missing semicolon reported at the end of the statement that is missing it. Eight of 1.5's stricter checks also return, from duplicate macro definitions to reading the value of a `thread` call, each measured against all five games' shipped scripts before being given a severity.
- In-source suppression, carried inside comments: `#pragma disable|restore <code>|all|format` silences a diagnostic per line, per code, or entirely — errors included, wider than the C# pragma the spelling comes from. 1.5's `// gscode ignore` still works as a legacy alias.
- `shift+F1` opens the API library page for the built-in function under the cursor, and commands cover showing the server log, restarting the server, and clearing the cache to reindex.
