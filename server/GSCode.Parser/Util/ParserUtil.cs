...
namespace GSCode.Parser.Util
{
    internal static class ParserUtil
    {
        ...
        public static string? GetScriptFilePath(string currentScriptPath, string desiredScriptPath, string? extension = null)
        {
            ...
            Log.Information("{0}", normalisedCurrentScriptPath);
            Log.Information("{0}", baseDir);
            Log.Information("{0}", scriptPath);

            string? basePath = null;
            if (normalisedCurrentScriptPath.Contains(baseDir))
            {
                Log.Information("{baseDir} found", baseDir);
        ...
        public static string? GetCommentContents(string? commentContents, CommentTypes? commentType)
        {
            if (commentContents == null || commentType == null)
            {
                return null;
            }
            ...
            contentSpan = commentType == CommentTypes.Line ? contentSpan[sliceIndex..] : contentSpan.Slice(sliceIndex, contentSpan.Length - sliceIndex - 2);
        ...
        public static string ProduceSnippetString(List<Token> tokensSource)
        {
            ...
            int startIndex = tokenSpan[0].Is(TokenType.Whitespace) ? 1 : 0;
            int endIndex = tokenSpan[^1].Is(TokenType.Whitespace) ? tokenSpan.Length - 1 : tokenSpan.Length;

            for (int i = startIndex; i < endIndex; i++)
            {
                sb.Append(tokensSource[i].Contents);
            }
