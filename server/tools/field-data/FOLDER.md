...
### CoD4 (the one non-BO3 game with bundled data)

- `cod4_wordfile.txt` — the mod-tools syntax-highlighting wordfile (`bin\wordfile.txt`).
  Its `/L11 "CODSCRIPT"` block is CoD4's only list of engine functions and entity
  fields without an API: `/C7 "Script Commands"` → builtin function names,
  `/C2 "Common Entity Properties"` → entity `.fields`. The tool parses both.
- `cod4_keys.txt` — CoD4's `raw\radiant\keys.txt` (same format as the BO3 one above);
  the tool runs it through the shared radiant-keys parser, so CoD4 keys carry types.
