...
export class Editor {
	...
	/** Cheap stats — only reads flags and dirty state, no validation */
	readonly stats = $derived.by(() => {
		...
		for (const fn of this.functions.values()) {
			...
			// Read flags directly — avoids triggering $derived chains for each entry
			const flags = fn.function.flags;
			const isVerified = flags.includes('verified');
			const isProcessed = flags.includes('processed');
			// The pre-BO3 libraries use their own vocabulary. `documented` means a real per-function
			// page stands behind the entry — the strongest provenance any library here has — so it
			// must not fall through into the "nothing is known about this" bucket, which is what
			// counting only BO3's two flags did to Call of Duty 4's 791 documented functions.
			const isDocumented = flags.includes('documented');

			if (fn.isNew) {
				newCount++;
			}
			if (!isVerified && !isProcessed && !isDocumented) {
				autogeneratedCount++;
			}
			if (isProcessed) {
				processedCount++;
	...
	/**
	 * Computes validation stats on demand — expensive, iterates all functions.
	 * Call this explicitly (e.g. on a button click) rather than in a reactive derived.
	 */
	computeValidationStats(): { invalidCount: number; badVerificationCount: number } {
		let invalidCount = 0;
		...
		for (const fn of this.functions.values()) {
			...
			if (fn.isInvalid) {
				invalidCount++;
				if (fn.function.flags.includes('verified')) {
					badVerificationCount++;
				}
			}
		}

		return { invalidCount, badVerificationCount };
	}
	...
	updateLibrary(library: ScrLibrary) {
		...
		for (const fn of library.api) {
			this.functions.set(
				fn.name.toLowerCase(),
				new FunctionEditor(fn, false, () => this.markChanged())
			);
		}
	...
	saveToStorage() {
		...
		try {
			const state = {
				library: {
					...this.library,
					revisedOn: this.library.revisedOn ? this.library.revisedOn.toISOString() : null
				},
