<script lang="ts">
	...
	import { games, languagesFor } from '$lib/data/games';
	...
	function handleSaveToJSON() {
		...
		// Convert dates back to strings for JSON. A library that arrived without an envelope has no
		// date to convert, and inventing one would claim a revision history it does not have.
		const { provenance: _provenance, ...body } = exportedLibrary;
		const json = JSON.stringify(
			{
				...body,
				revisedOn: exportedLibrary.revisedOn ? exportedLibrary.revisedOn.toISOString() : null
			},
			null,
			4
		);
	...
	async function loadFromApiSource(gameSlug: string, languageId: 'gsc' | 'csc') {
		if (stats.editedCount > 0) {
			const confirmLoad = confirm(
		...
		loadingSource = `${gameSlug}-${languageId}`;
		try {
			const res = await fetch(`/api/getLibrary?gameId=${gameSlug}&languageId=${languageId}`);
			if (!res.ok) {
				...
				<dl class="flex flex-col gap-1 font-mono text-2xs">
					...
					<div class="flex items-baseline justify-between gap-3">
						...
						<dd class="text-foreground">
							{library.revisedOn ? library.revisedOn.toLocaleDateString() : 'unrecorded'}
						</dd>
					</div>
				</dl>
			{:else}
		...
		{#if hasLibrary}
			<div>
				...
				<dl class="flex flex-col gap-1 font-mono text-2xs">
					...
					{#if stats.documentedCount > 0}
						<div class="flex items-baseline justify-between gap-3">
							<dt class="text-muted-foreground">Documented</dt>
							<dd class="text-foreground">{stats.documentedCount}</dd>
						</div>
					{/if}
					<div class="flex items-baseline justify-between gap-3">
		...
		<div>
			...
			<div class="flex gap-2">
				<Popover.Root bind:open={loadPopoverOpen}>
					...
					<Popover.Content class="w-60 gap-0" align="start">
						<div class="flex max-h-80 flex-col overflow-y-auto">
							{#each games as game (game.slug)}
								<p class="type-label text-dim px-2 pt-2 pb-1">{game.shortName}</p>
								{#each languagesFor(game) as languageId (languageId)}
									<button
										type="button"
										...
										onclick={() => {
											loadFromApiSource(game.slug, languageId);
											loadPopoverOpen = false;
										}}
									>
										<Download class="size-3.5" />
										Load latest {languageId.toUpperCase()}
									</button>
								{/each}
							{/each}
