...
{:else if functionEditor.function.example}
	<button type="button" onclick={startEditing} class="group relative w-full cursor-pointer text-left">
		...
		<div class="absolute inset-0 flex items-center justify-center">
			<span
				class="chip-cut bg-popover text-foreground inset-edge flex items-center gap-2 px-3 py-2 font-mono text-[11px] tracking-[.13em] uppercase opacity-0 transition-opacity group-hover:opacity-100"
			>
...
{:else}
	...
	>
		<Plus class="size-5" />
		<span class="font-mono text-[11px] tracking-[.13em] uppercase">Add example code</span>
	</button>
{/if}
