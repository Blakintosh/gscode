<script lang="ts" module>
	 ...
	 *
	 * Version strings are not badges — they are `font-mono text-[10px] text-primary`.
	 */
	export const badgeVariants = tv({
		base: "group/badge inline-flex h-5 w-fit shrink-0 items-center justify-center gap-1.5 px-2.5 font-mono text-[9px] leading-none tracking-[.14em] whitespace-nowrap uppercase transition-[filter,background,color] duration-150 outline-none focus-visible:[box-shadow:inset_0_0_0_2px_var(--ring)] [&>svg]:pointer-events-none [&>svg]:size-3",
		variants: {
			variant: {
				default: "badge-cut bg-primary text-primary-foreground [a]:hover:brightness-[1.08]",
				secondary: "badge-cut bg-popover text-muted-foreground inset-edge [a]:hover:text-foreground",
